...
class BladeGameApp:
    ...
    def load_ai_avatar(self, maid_name):
        ...
        if os.path.exists(path):
            try:
                ...
                self.ai_img_label.image = ImageTk.PhotoImage(img)
            except:
                pass
    ...
    def create_widgets(self):
        ...
        # 状态指示器（新增）
        self.status_indicator_frame = tk.Frame(self.root, bg="#e8f5e9", pady=8)
        self.status_indicator_frame.pack(fill="x", padx=20)
        
        self.status_icon = tk.Label(self.status_indicator_frame, text="⏳", font=("Arial", 16), bg="#e8f5e9")
        self.status_icon.pack(side="left", padx=10)
        
        self.status_label = tk.Label(self.status_indicator_frame, text="等待回合...", font=("Arial", 11), bg="#e8f5e9", fg="#666")
    ...
    def update_ui(self, state):
        ...
        if state.get("game_over"):
            self.status_indicator_frame.config(bg="#ffebee")
            self.status_icon.config(text="🏁", bg="#ffebee")
            self.status_label.config(text="游戏结束", bg="#ffebee", fg="#c62828")
            self.disable_all_buttons()
        elif state.get("user_ready"):
            self.status_indicator_frame.config(bg="#fff3e0")
            self.status_icon.config(text="⚔️", bg="#fff3e0")
            self.status_label.config(text="招式已发送！等待 AI 回应...", bg="#fff3e0", fg="#e65100")
            self.disable_all_buttons()
        else:
            self.status_indicator_frame.config(bg="#e8f5e9")
            self.status_icon.config(text="🎯", bg="#e8f5e9")
            self.status_label.config(text="请选择你的招式", bg="#e8f5e9", fg="#2e7d32")
    ...
    def on_move_click(self, move_key):
        ...
        try:
            ...
            # 立即更新UI反馈
            self.status_indicator_frame.config(bg="#e3f2fd")
            self.status_icon.config(text="✅", bg="#e3f2fd")
            self.status_label.config(
                text=f"已选择 [{MOVES_INFO[move_key]['name']}]，正在同步...", 
                bg="#e3f2fd", 
                fg="#1565c0"
            )
            self.disable_all_buttons()
            ...
            # 短暂延迟后切换到等待状态
            self.root.after(500, lambda: self.update_ui(self.state))
